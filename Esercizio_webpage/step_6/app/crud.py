...
# Funzioni per la newsletter
def subscribe_to_newsletter(db: Session, email: str):
    # Prima controlla se l'indirizzo email è già iscritto
    existing_subscriber = db.query(database.NewsletterSubscriber).filter(
        database.NewsletterSubscriber.email == email
    ).first()
    
    # Se esiste ma non è attivo, lo riattiviamo
    ...
    # Se non esiste, creiamo un nuovo abbonato
    new_subscriber = database.NewsletterSubscriber(email=email)
...
def unsubscribe_from_newsletter(db: Session, token: str):
    # Cerca l'abbonato con il token specificato
    subscriber = db.query(database.NewsletterSubscriber).filter(
        database.NewsletterSubscriber.unsubscribe_token == token
    ).first()
...
def get_active_newsletter_subscribers(db: Session):
    # Restituisce tutti gli abbonati attivi alla newsletter
    return db.query(database.NewsletterSubscriber).filter(
        database.NewsletterSubscriber.is_active == True
    ).all()


def get_subscriber_by_email(db: Session, email: str):
    # Cerca un abbonato per email
    return db.query(database.NewsletterSubscriber).filter(
        database.NewsletterSubscriber.email == email
    ).first()
...
def get_subscriber_by_token(db: Session, token: str):
    # Cerca un abbonato per token di disiscrizione
    return db.query(database.NewsletterSubscriber).filter(
        database.NewsletterSubscriber.unsubscribe_token == token
    ).first()
...
def update_user_email(db: Session, user_id: int, new_email: str):
    ...
    # Verifica prima se l'email è già in uso
    existing_user = db.query(database.User).filter(
        database.User.email == new_email,
        database.User.id != user_id
    ).first()
    
    if existing_user:
        return False, "Email già in uso da un altro account"
    ...
    db.commit()
    return True, "Email aggiornata con successo"
...
def get_user_reviews(db: Session, username: str):
    """Ottiene tutte le recensioni scritte da un utente specifico."""
    return db.query(database.Review).filter(database.Review.user == username).order_by(database.Review.created_at.desc()).all()

def delete_review(db: Session, review_id: int):
    """Elimina una recensione dal database e il prodotto se non ha più recensioni."""
    review = db.query(database.Review).filter(database.Review.id == review_id).first()
    if review:
        ...
        # Controlla se il prodotto ha ancora recensioni
        remaining_reviews = db.query(database.Review).filter(database.Review.product_id == product_id).count()
        
        # Se non ci sono più recensioni, elimina il prodotto
...
def update_review(db: Session, review_id: int, title: str, rating: int, description: str):
    """Aggiorna una recensione esistente.
    
    Args:
        db: Sessione del database
        review_id: ID della recensione da aggiornare
        title: Nuovo titolo
        rating: Nuovo rating
        description: Nuova descrizione
        
    Returns:
        La recensione aggiornata o None se non è stata trovata
    """
    review = db.query(database.Review).filter(database.Review.id == review_id).first()
    if not review:
        return None
